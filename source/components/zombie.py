...
class Zombie(pg.sprite.Sprite):
    ...
    def load_frames(self, frames, name, image_x, colorkey=c.BLACK):
        ...
        for frame in frame_list:
            frames.append(engine.get_image(frame, image_x, 0, width, height, colorkey))

    def update(self, game_info):
        self.current_time = game_info[c.CURRENT_TIME]
        self.handle_state()
        self.update_ice_slow()
        self.animation()
    ...
    def walking(self):
        if self.health <= 0:
            ...
            self.change_frames(self.losthead_walk_frames)
            self.set_lost_head()
        elif self.health <= c.NORMAL_HEALTH and self.helmet:
            self.change_frames(self.walk_frames)
            self.helmet = False
            if self.name == c.NEWSPAPER_ZOMBIE:
                self.speed = 2

        if (self.current_time - self.walk_timer) > (c.ZOMBIE_WALK_INTERVAL * self.get_time_ratio()):
    ...
    def attacking(self):
        if self.health <= 0:
            ...
            self.change_frames(self.losthead_attack_frames)
            self.set_lost_head()
        elif self.health <= c.NORMAL_HEALTH and self.helmet:
            self.change_frames(self.attack_frames)
            self.helmet = False
        if (self.current_time - self.attack_timer) > (c.ATTACK_INTERVAL * self.get_time_ratio()):
            if self.prey.health > 0:
                if self.prey_is_plant:
                    self.prey.set_damage(self.damage, self)
                else:
                    self.prey.set_damage(self.damage)
    ...
    def animation(self):
        ...
        if (self.current_time - self.animate_timer) > (self.animate_interval * self.get_time_ratio()):
            self.frame_index += 1
    ...
    def set_walk(self):
        ...
        if self.helmet:
            self.change_frames(self.helmet_walk_frames)
        elif self.losHead:
            self.change_frames(self.losthead_walk_frames)
        else:
            self.change_frames(self.walk_frames)

    def set_attack(self, prey, is_plant=True):
        ...
        if self.helmet:
            self.change_frames(self.helmet_attack_frames)
        elif self.losHead:
            self.change_frames(self.losthead_attack_frames)
        else:
            self.change_frames(self.attack_frames)
    ...
    def set_die(self):
        ...
        self.change_frames(self.die_frames)
    
    def set_boom_die(self):
        self.state = c.DIE
        self.animate_interval = 200
        self.change_frames(self.boomdie_frames)

    def set_freeze(self, ice_trap_image):
    ...
    def draw_freeze_trap(self, surface):
        if self.state == c.FREEZE:
...
class ZombieHead(Zombie):
    ...
    def load_images(self):
        self.die_frames = []
        die_name =  self.name
        self.load_frames(self.die_frames, die_name, 0)
...
class NormalZombie(Zombie):
    ...
    def load_images(self):
        ...
        for i, name in enumerate(name_list):
            self.load_frames(frame_list[i], name, engine.ZOMBIE_RECT[name]['x'])
    ...
    def load_images(self):
        ...
        for i, name in enumerate(name_list):
            if name == c.BOOMDIE:
                ...
                color = c.WHITE
            self.load_frames(frame_list[i], name, engine.ZOMBIE_RECT[name]['x'], color)
